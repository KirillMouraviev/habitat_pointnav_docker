...
class HabitatRunner():
    def __init__(self):
        ...
        self.dataset_save_path = '/data/datasets/opr_training_data/hm3d_train'
    ...
    def run_episode(self, ii):
        ...
        poses = []
        i = 0
        if not os.path.exists(os.path.join(self.dataset_save_path, str(ii))):
            os.mkdir(os.path.join(self.dataset_save_path, str(ii)))
        #else:
        #    return
        for pt in tqdm(points):
            for ori in orientations:
                i += 1
                observations = self.env.reset(i=ii, start_position=pt, start_orientation=ori)
                rgb = observations['rgb']
                depth = observations['depth']
                depth = (depth * 255).astype(np.uint8)
                poses.append(pt + ori)
                imsave(os.path.join(self.dataset_save_path, str(ii), '{}_rgb.png'.format(i)), rgb)
                imsave(os.path.join(self.dataset_save_path, str(ii), '{}_depth.png'.format(i)), depth)
        np.savetxt(os.path.join(self.dataset_save_path, str(ii), 'poses.txt'), np.array(poses))
