...
class Mapper:
    ...
    def reset(self):
        ...
        self.last_sim_location_old = [0, 0, 0]
        self.curr_loc_gt = [self.map_size_cm/100.0/2.0,
                         self.map_size_cm/100.0/2.0, 0.]
        self.curr_loc_gt_old = [self.map_size_cm/100.0/2.0,
                         self.map_size_cm/100.0/2.0, 0.]
    ...
    def step(self, observations, semantic_mask):
        ...
        dx_gt, dy_gt, do_gt = self.get_gt_pose_change_old(list(observations['gps_old']*np.array([1,-1]))+list(observations['compass_old']))
        self.curr_loc_gt_old = pu.get_new_pose(self.curr_loc_gt_old,(dx_gt, dy_gt, do_gt))
        ...
        depth_old = self._preprocess_depth(observations['depth_old'])
        old_pose = (self.curr_loc_gt_old[0]*100.0,self.curr_loc_gt_old[1]*100.0,np.deg2rad(self.curr_loc_gt_old[2]))
        ...
        fp_proj, self.map, fp_explored, self.explored_map, fp_semantic, self.semantic_map = \
            self.mapper.update_map(depth, depth_old, mapper_gt_pose, old_pose, semantic_mask, 0) #observations['semantic'][:,:,0]
    ...
    def get_gt_pose_change_old(self,loc):
        curr_sim_pose = loc
        dx, dy, do = pu.get_rel_pose_change(curr_sim_pose, self.last_sim_location_old)
        self.last_sim_location_old = curr_sim_pose
        return dx, dy, do
