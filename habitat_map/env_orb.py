...
class Env:
    ...
    def __init__(
        ...
        #)
        print(config)
        self._config = config.habitat
        #self.dt = self._config.simulator.turn_angle
        self._dataset = dataset
        self._current_episode_index = None
        self._config.dataset.split = 'train'
        self._config.dataset.data_path = '/data/datasets/objectnav/hm3d/v2/train/train.json.gz'
        print(self._config.dataset.split, self._config.dataset.data_path)
        ...
        episode = self._dataset.episodes[0]
        self._episodes = (
        ...
        print('START ROTATION:', self._episode_iterator.episodes[0].start_rotation)
    ...
    def get_navigable_points(self):
        points = []
        y = self._current_episode.start_position[1]
        for x in range(-30, 30):
            for z in range(-30, 30):
                if self._sim.is_navigable([x, y, z]):
                    points.append([x, y, z])
        return points

    def reset(self, i=None, goal_reacher=True, start_position=None, start_orientation=None) -> Observations:
        ...
        if start_position is not None:
            self._current_episode.start_position = start_position
        if start_orientation is not None:
            self._current_episode.start_rotation = start_orientation
    ...
    def step(
        ...
        observations = self._task.step(action=action, episode=self._current_episode)
        self.steps+=1
        #self.obs = observations
        self._task.measurements.update_measures(episode=self._current_episode, observations=observations, action=action, task=self._task)
    ...
    def reconfigure(self, config: DictConfig) -> None:
        self._config = config
        OmegaConf.set_readonly(self._config, False)
        self._config = self._task.overwrite_sim_config(
            self._config, self._current_episode
        )
